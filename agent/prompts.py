"""Conversion system prompts.

scenario.json (tool-neutral IR) -> (a) natural-language steps, (b) Appium Python,
(c) Maestro YAML.

Design principle: the generated code *never calls an LLM at execution time*. It is
deterministic, resource-id based, and coordinate-free. (Driving every step with
vision would be a runtime cost/latency bottleneck, so it is avoided.)
"""

# Shared input description. Used by all prompts.
SCHEMA_NOTE = """\
...
2. Describe intent in plain, natural English: navigate -> "Open <url>",
   click -> "Click <element>", change -> 'Enter "<value>" in <field>',
   keyDown Enter -> "Press Enter", waitForElement -> "Verify <element> is visible".
   Name the element in a human way from its selector (e.g. ".new-todo" -> "the new-todo input").
3. Skip pure setViewport and keyUp steps (or fold them in).
4. Output plain text only, no preamble, no markdown, no code fences. Do NOT
   comment on the schema — just produce the numbered steps.
"""
...
OUTPUT: a JSON array of exactly N objects, each:
  {"title": "<short title (3-6 words)>",
   "desc": "<1-2 sentence description of what this scenario tests>",
   "brief": "<one concrete instruction for a code generator>"}
- Output ONLY the JSON array. No prose, no markdown fences.
Example:
[{"title":"Complete 3 todos","desc":"Add three todos and complete the middle one, verifying the completed state is reflected.","brief":"Add three todos then complete the middle one and verify it is marked completed"}]
"""

# Brief (one English sentence) -> complete Playwright script. Generated by the LLM inside the runtime.
